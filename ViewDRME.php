    ...
    <body class="skin-blue sidebar-mini">
                                        ...
                                        <div class="register-box-body">
                                            <table id="table_site" class="table table-bordered table-striped table-condensed">
                                                ...
                                                <tbody>
                                                    <?php
                                                    include './DBCon.php';
                                                    $sql = "SELECT * FROM drme WHERE status!=0";
                                                    ...
                                                    if (mysqli_num_rows($result) > 0) {
                                                        while ($row = mysqli_fetch_assoc($result)) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <?php
                                                                $sql1 = "SELECT inventory.code FROM inventory INNER JOIN drc ON inventory.id=$row[idinventory]";
                                                                ...
                                                                <td>
                                                                    <button type="button" class="btn btn-xs btn-default" onclick="showActivity(<?php echo $row["id"]; ?>)"><span class="glyphicon glyphicon-th-list" aria-hidden="true"></span></button>
                                                                </td>
                                                                <td>
                                                                    <?php if ($row['status'] == '1') { ?>
                                                                        <form action="db/ChangeStatus.php" method="POST">
                                                                            <input type="hidden" name="id" value="<?php echo $row['id']; ?>"/>
                                                                            <input type="hidden" name="table" value="drme"/>
                                                                            <input type="hidden" name="url" value="ViewDRME"/>
                                                                            <div class="btn-group">
                                                                        ...
                                                                        <div class="btn-group">
                                                                            <?php if (isAdmin()) { ?>
                                                                                ...
                                                                                <ul class="dropdown-menu" role="menu">
                                                                                    <li><a href="db/ChangeStatus.php?id=<?php echo $row['id']; ?>&status=0&table=drme&url=ViewDRME">Approve</a></li>
                                                                                    <li><a href="db/ChangeStatus.php?id=<?php echo $row['id']; ?>&status=1&table=drme&url=ViewDRME">Cancel</a></li>
                                                                                </ul>
                                                                            <?php } else { ?>
                                                                                <button type="button" class="btn btn-warning btn-flat btn-xs" disabled>Pending</button>                                                  
                                                                            <?php } ?>
                                                                ...
                                                                <td>
                                                                    <form action="UpdateDRME.php" method="POST">
                                                                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>"/>
        ...
        <script>
            function showActivity(id) {
                $('#modalWindow').empty();
                $('#modalWindow').load("ViewDRMEActivity.php?id=" + id);
                $('#modalButton').click();
            }
        </script>
